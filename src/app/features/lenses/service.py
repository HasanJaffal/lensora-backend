...
from app.features.lenses.pricing import (
...
class LensCatalogService:
    ...
    async def get_catalog(self) -> LensCatalogDto:
        return LensCatalogDto(
            ...
            materials=[
                LensOptionDto.from_model(o) for o in await self._catalog.list_materials()
            ],
            coatings=[
                LensOptionDto.from_model(o) for o in await self._catalog.list_coatings()
            ],
...
class OrderService:
    ...
    def __init__(
        ...
        *,
        ...
        self._deposit_percent = deposit_percent
    ...
    async def create_order(self, request: CreateOrderRequest) -> OrderDto:
        async with UnitOfWork(self._sessionmaker) as uow:
            patients = PatientRepository(uow.session)
            inventory = InventoryRepository(uow.session)
            catalog = LensCatalogRepository(uow.session)
            ...
            priced = _price(
                lens_type, material, coatings, tint, frame, self._deposit_percent
            )
            order = _build_order(request.patient_id, priced, frame)
            orders.add(order)
    ...
    async def get_current_order_for_patient(self, patient_id: uuid.UUID) -> OrderDto:
        patient = await PatientRepository(self._session).get_by_id(patient_id)
        if patient is None:
            raise PatientNotFoundError(f"Patient {patient_id} not found")
        order = await OrderRepository(self._session).get_latest_for_patient(patient_id)
        if order is None:
...
def _price(
    ...
    items.extend(
        LineItem(coating.name_en, coating.name_ar, coating.price) for coating in coatings
    )
...
def _build_order(
    patient_id: uuid.UUID, priced: PricedOrder, frame: InventoryItem
) -> Order:
    return Order(
