...
def get_intake_service(session: SessionDep) -> IntakeService:
    return IntakeService(IntakeRepository(session))
...
async def list_intakes(
    ...
    return ok(
        await service.list_intakes(
            patient_id=patient_id, status=_parse_status(status)
        )
    )
...
async def get_intake(
    intake_id: uuid.UUID, service: IntakeServiceDep
) -> Envelope[IntakeDto]:
