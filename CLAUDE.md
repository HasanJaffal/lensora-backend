...
- `UserRole` has exactly one value: **`ORGANIZATION_ADMIN`** — the single account per organization, with full access to its own tenant. There is no permission table and no second role; "authenticated" is equivalent to "authorized within the tenant."
- **Platform administrators are out-of-band.** There is no `PLATFORM_ADMIN` role or authenticated route. Provisioning a new organization is a direct-DB/CLI operation (`python -m app.management.provision_organization`, see [`README.md`](README.md)) run by someone with server access, not an API principal.
- Role assignment happens once, at provisioning time. There is no self-registration, no user-management UI, and no invitation flow.
