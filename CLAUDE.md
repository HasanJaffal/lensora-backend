...
1. **SOLID, always.**
   ...
   - *Liskov* — substitutable implementations honor the same contract (a fake object storage must be a drop-in for the Supabase one).
...
- **Migrations are code-first.** Change models, autogenerate with Alembic, then **review the migration by hand** (renames autogenerate as drop+add). One migration per logical change; never edit an applied migration. Seed data lives in `db/seeds/`, never in migrations.
...
- **Two repositories intentionally sit outside `TenantScopedRepository`**, and only these two. `UserRepository` is **unscoped**: login resolves an account by email before any tenant is known, so it is the sole place a feature may query the `user_account` table without a tenant filter. `StorefrontProductRepository` ([`features/storefront/repository.py`](src/app/features/storefront/repository.py)) is **scoped by explicit argument rather than by context**: an anonymous storefront visitor has no account to derive a `TenantContext` from, so the organization boundary is resolved from the requested slug and passed in as `organization_id`, which every query filters on. It is never unscoped. No other repository gets either exemption.
...
- **`PLATFORM_ADMIN`** is a single account with `organization_id = NULL`. It authenticates through the same `/auth/login` as any other account, but `UserDto`/`LoginResponse` carry `organization: null`, and `GET /auth/me` reflects the same. It is the **only account the application seeds**, created at startup from `PLATFORM_ADMIN_EMAIL` / `PLATFORM_ADMIN_PASSWORD` (plus optional bilingual display names) by [`db/seeds/platform_admin.py`](src/app/db/seeds/platform_admin.py), wired through the lifespan hook in [`main.py`](src/app/main.py). The environment is the source of truth: every boot re-applies the configured credentials to the existing account instead of creating a second one, so rotation is an env change plus a restart. Seeding is skipped when either value is blank, and fails loudly with `auth.emailTaken` if the configured email already belongs to an organization admin. There is no self-registration and no in-app creation of additional platform admins.
...
- Role assignment happens once, at creation time — from the environment for the platform admin, and at provisioning time for an organization admin.
...
- **No demo data ships in the application.** The only seeded rows are the platform admin and a newly provisioned organization's read-only catalog/tips defaults; there are no pre-created organizations. The deterministic patient/inventory datasets the suite builds tenants from live in `tests/fixtures/` and must never be imported by `src/`.
